...
module_param(how_many_timesy, uint, 0444);
MODULE_PARM_DESC(how_many_times, "Number of times the message is printed");
...
static int __init hello_init(void)
{
	if (how_many_times == 0 || (how_many_times >= 5 && how_many_times <= 10)) {
		...
		printk(KERN_WARNING "Error: how_many_times is more then 10\n");
		return -EINVAL;
	}
